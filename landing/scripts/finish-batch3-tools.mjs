...
const tools = JSON.parse(fs.readFileSync("scripts/batch3-tools-meta.json", "utf8"))
...
<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  ${textSvg}
  <text x="72" y="560" font-family="Arial, Helvetica, sans-serif" font-size="24" fill="#64748b">Free online tool</text>
</svg>`
...
const missing = tools.filter((t) => !toolsTs.includes(`slug: "${t.slug}"`))
if (missing.length) {
  ...
  console.log("registry appended", missing.length)
} else {
  console.log("registry already complete")
}
