...
const tools = JSON.parse(fs.readFileSync("scripts/lifestyle-tools-meta.json", "utf8"))
...
if (!toolsTs.includes("bmi-calculator")) {
  const entries = tools
    .map(
      (t) => `  {
  ...
  console.log("registry already has bmi-calculator — skipped")
}

let sitemap = fs.readFileSync("public/sitemap.xml", "utf8")
if (!sitemap.includes("bmi-calculator")) {
  ...
  console.log("sitemap already has bmi-calculator — skipped")
}
